...
class userController extends Controller
{
    ...
    $renewalDate = $createdDate->addYear();
      ...
      $renewals=Renewals::create($data1);
    ...
    DB::rollback(); // Rolling back in case of an exception/error
...
if ($client_value && $device) {
 ...
 public function allcomplain(){
    ...
    ->paginate(8);
    $data_1=complain::where('Status','resolved')
    ->orderBy('created_at','desc')
    ->paginate(8);


    return view ('allcomplain',['data'=>$data,'data_1'=>$data_1]);
...
public function datalogs($search_term){
 ...
 public function create_emp(Request $request){
    ...
    Employee::create($emp);
   return response()->json('Employee adedd successfully', 200,);
 }
...
public function create_update_emp(Request $request){
    ...
    if($emp){
        ...
        return response()->json([
            'success'=>true,
            'message'=>"Employee updated successfuily"
        ], 200, );
    }
    else{
        return response()->json([
...
public function emp_login(Request $request)
{
    ...
    if ($request->session()->has('emp_id_' . $request->session()->get('session_token'))) {
        // An employee is already logged in, redirect or display a message
        return view('error')->with('message', 'Another employee is already logged in. Please logout first.');
    }
   ...
   // Inside your login function
     ...
     $uniqueSessionToken = 'session_token_' . $empId . '_' . time(); // Unique token based on employee ID and timestamp

     // Store employee details with the session token
     $request->session()->put('emp_id_' . $uniqueSessionToken, $empId);
     $request->session()->put('emp_name_' . $uniqueSessionToken, $empname);
     $request->session()->put('em_loginid_' . $uniqueSessionToken, $loginid);
     $request->session()->put('session_token', $uniqueSessionToken);
    ...
    if ($empid) {
        $data = [
            'emp_id' => $empid->emp_id,
            'Emp_name' => $empid->emp_name,
            'login_id' => $request->em_loginid,
            // 'designation'=>$request->designation,
            'status' => 'active',
        ];

        // Create a new Emp_login record
       $emp_login= Emp_login::create($data);
    }
    if($emp_login){
        $request->session()->put('id',$emp_login->id);
        $request->session()->put('designation',$empid->designation);

    }
$emp_role=$employee->role;
    // Redirect logic based on user credentials

        if ($emp_role==='Head') {
            return view('/home3',compact('empid'));
        }
        elseif ($emp_role==='Technical') {
            return view('/home1',compact('empid'));
        } elseif ($emp_role==='Super Visor') {
            return view('/home2',compact('empid'));
        }
        elseif ($emp_role==='CRO'||'Sales Operator'||'Sales Person'||'Technician') {
                  return view('/home',compact('empid'));
            }
}
    ...
    public function getid( Request $request){
        ...
        if ($empid) {
            ...
            // Create a new Emp_login record
            Emp_login::create($data);
        }

       if($employee) {
            // Redirect authenticated users to a default home page
            return response()->json([
                'success' => true,
                'message'=>'loggd in successsfully',
                   'emp_name' => $empid->emp_name,
            ]);
        }
    }
